...
In R, a function is piece of code that performs a specific task. Functions take inputs (called arguments) and return outputs. For example, the `mean()` function takes a series of numerical values as the input and returns calculates their average. Try running the following code in the console:

```{r}
mean(3,4,5)
```

Notice that this function takes three arguments (3, 4, and 5) and returns the mean of those numbers as the output. The general syntax for a function is `function_name(argument1, argument2, ...)`. In this case, `mean` is the function name and `3`, `4`, and `5` are the arguments.
...
Suppose we want to find the mean of the numbers 23,24,26,19,18,25,21, and 39. While it might be easy to use the function `mean(23,24,...)`, this might pose problems if the list is long. A common thing to do is to create a container to put all these numbers. The container is known as a vector. See coed below for creating a container for the list above:
