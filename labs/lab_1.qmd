...
The inputs of a function are called `arguments`. Here, we have three arguments (3, 4, and 5). Note that the arguments are separated by commas. The general syntax for a function is `function_name(argument1, argument2, ...)`. In this case, `mean` is the function name and `3`, `4`, and `5` are the arguments.
